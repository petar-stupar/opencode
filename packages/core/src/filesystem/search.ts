...
// Import the shared schema directly rather than through `../filesystem`. The parent module
// depends on this one for its layer node, so a runtime import back would create a cycle that
// the bundler resolves with `FileSystemSearch.node` still undefined.
